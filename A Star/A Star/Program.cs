...
class aStar
{
    ...
    public double distance(Node currentNode, Node previousNode)
    {
        double startX = currentNode.location[0];
        double startY = currentNode.location[1];

        double endX = currentNode.location[0];
        double endY = currentNode.location[1];
    ...
    public void processConnections(Node currentNode, Node nodeBeingProcessed, Node endNode, List<Node> openList)
    {
        for (int i = 0; i < currentNode.connectedNodes.Count; i++)
        {
            nodeBeingProcessed = currentNode.connectedNodes[i];
            nodeBeingProcessed.parentNode = currentNode;
            Node processedNode = processNode(nodeBeingProcessed, currentNode, endNode);
    ...
    public void testNode(Node currentNode, Node startNode, Node endNode, List<Node> closedList)
    {
        ...
        if (currentNode.name == startNode.name)
        {
            ...
            processConnections(currentNode, current, endNode, openList);
        }
